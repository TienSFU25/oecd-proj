...
def main():
    ...
    corr_mat, labels, cats, countries = getCorrelation_cat(processed_dict)
    ...
    for idx,ci in enumerate(cats[0]):
        ...
        for idy,cj in enumerate(countries):
            temp = {}
            for idz, cz in enumerate(countries):
                temp[cz] = corr_mat[idx][idy][idz]

            tTemp[cj] = temp
        #print(tTemp)
        dataFinal[ci] = tTemp

    with open('parsed_corr_data.json', 'w') as f:
